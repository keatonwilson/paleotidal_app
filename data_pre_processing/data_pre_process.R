...
combine_all_years = function(data_dir, 
  ...
  # stratification requires special processing
  ...
  if (!is_bss & !is_strat) {
    ...
    # group split
    files_df = tibble::tibble(filename = files_to_read) |> 
      dplyr::mutate(type = dplyr::case_when(stringr::str_detect(filename, "log10_strat") ~ "log10", 
                                            stringr::str_detect(filename, "/strat_") ~ "absolute")) |> 
      dplyr::group_split(type)
